...
export default function Home() {
  ...
  return (
    <div className="p-4">
      ...
      {!loading && tracks.length === 0 && <p className="text-slate-400">No tracks yet — be the first to upload one.</p>}

      {featured.length > 0 && (
        <div className="flex gap-3 overflow-x-auto pb-4 mb-6 -mx-4 px-4">
          {featured.map((track) => (
            <div key={track.id} onClick={() => playTrack(track)} className="flex-shrink-0 w-40 cursor-pointer">
